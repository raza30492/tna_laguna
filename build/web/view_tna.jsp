                            ...
                            <c:if test="${requestScope.buyerTNAList != null}" >
                                ...
                                <c:if test="${requestScope.message == null}">
                                    <div style="width: 50%;margin: 20px auto" >
                                        ...
                                        <table class="table-bordered" id="bordered-table" style="width: 100%;">
                                            ...
                                            <c:forEach items="${requestScope.buyerTNAList}" var="tna">
                                                <tr>
                                                    <td><a href="view_po?redirect=three&page=view&poRef=${tna.key}">${tna.key}</a></td>
                                                    <td>${tna.value}</td>
                                                </tr>
                                            </c:forEach>
                                    ...
                                    <div id="paging" >   <!-- Pagination Div -->
                                        ...
                                        <c:if test="${requestScope.curr != 1}">
                                            <div id="pagination" > <a href="view_po?redirect=two&view=allPORef&curr=${(requestScope.curr - 1)}&buyer=${requestScope.buyer}">Previous </a></div>
                                        </c:if>

                                        <c:forEach var="i" begin="1" end="${requestScope.total}" >
                                            <c:choose >
                                                <c:when test="${(i < requestScope.curr) && (i <= 3 || (requestScope.curr - i) <= 2)}">
                                                    <div id="pagination" ><a href="view_po?redirect=two&view=allPORef&curr=${i}&buyer=${requestScope.buyer}">${i}</a></div>
                                                </c:when>
                                        ...
                                        <c:if test="${(requestScope.curr != requestScope.total)}">
                                            <div id="pagination" > <a href="view_po?redirect=two&view=allPORef&curr=${(requestScope.curr + 1)}&buyer=${requestScope.buyer}">Next </a></div>
                                        </c:if>
