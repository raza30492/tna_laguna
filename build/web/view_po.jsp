    ...
    <body>
        ...
        <div id="wrapper" class="container" >
            ...
            <section id="tna-content" >
                ...
                <div>   <!--Activity Wise TNA div-->
                    <c:if test="${requestScope.activityWise != null}" >
                        <div style="margin: 20px auto">
                            <form class="form-inline" role="form" name="activityform" action="view_po" method="get" onsubmit="return selectActivity()">                               
                                <select name="activityNo" class="form-control">
                                    <option value="" selected>Select Activity</option>
                                    <c:forEach items="${applicationScope.allActivityMap}" var="activity" >
                                ...
                                <button type="submit" name="view" value="allActivity"  class="btn btn-default" >View</button>
                                <input type="hidden" name="redirect" value="two" /> 
                                <input type="hidden" name="from" value="tna" />
                            </form>
                        </div>
                    </c:if>
                ...
                <div>  <!-- PORef Wise Option Div-->  
                    <c:if test="${requestScope.poRefWise != null}" >
                        <div style="margin: 20px auto">
                            <form class="form-inline" name="form" action="view_po" method="get" onsubmit="return selectBuyer()">
                                <select name="buyer" class="form-control">
                            ...
                            <c:if test="${requestScope.buyerTNAList != null}" >
                                ...
                                <c:if test="${requestScope.message == null}">
                                    <div style="width: 50%;margin: 20px auto" >
                                        ...
                                        <table class="table-bordered" id="bordered-table" style="width: 100%;">
                                            ...
                                            <c:forEach items="${requestScope.buyerTNAList}" var="tna">
                                                <tr>
                                                    <td><a href="view_po?redirect=three&from=tna&page=view&poRef=${tna.key}">${tna.key}</a></td>
                                                    <td>${tna.value}</td>
                                                </tr>
                                            </c:forEach>
                                    ...
                                    <div id="paging" >   <!-- Pagination Div -->
                                        ...
                                        <c:if test="${requestScope.curr != 1}">
                                            <div id="pagination" > <a href="view_po?redirect=two&from=tna&view=allPORef&curr=${(requestScope.curr - 1)}&buyer=${requestScope.buyer}">Previous </a></div>
                                        </c:if>

                                        <c:forEach var="i" begin="1" end="${requestScope.total}" >
                                            <c:choose >
                                                <c:when test="${(i < requestScope.curr) && (i <= 3 || (requestScope.curr - i) <= 2)}">
                                                    <div id="pagination" ><a href="view_po?redirect=two&from=tna&view=allPORef&curr=${i}&buyer=${requestScope.buyer}">${i}</a></div>
                                                </c:when>
                                        ...
                                        <c:if test="${(requestScope.curr != requestScope.total)}">
                                            <div id="pagination" > <a href="view_po?redirect=two&from=tna&view=allPORef&curr=${(requestScope.curr + 1)}&buyer=${requestScope.buyer}">Next </a></div>
                                        </c:if>
                ...
                <div>   <!--Search TNA viv-->
                    <c:if test="${requestScope.searchPO != null}" >
                        <p id="errorBox"></p>
                        <form class="form-inline" role="form" name="searchform" action="view_po" method="get" onsubmit="return searchTNA()">
